...
exports.handler = async function(event, context) {
    ...
    try {
        ...
        if (!uid) {
            return { statusCode: 400, body: 'Bad Request: Missing user UID.' };
        }
        
        if (uid === requesterUid) {
            return { statusCode: 400, body: 'Bad Request: Cannot delete your own account.' };
        }
        ...
        return {
            statusCode: 500,
            body: JSON.stringify({ error: `Failed to delete user: ${error.message}` }),
        };
    }
};
